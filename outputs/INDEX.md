# 출력물 인덱스

> mobile-reservation-maestro-portfolio — Cycle 01 출력물 목록

## Cycle 01 — iOS Real Execution Verified

| 번호 | 문서 | 설명 | 링크 |
|------|------|------|------|
| 1 | **README.md** | 프로젝트 개요, 상태, 실행 방법, 테스트 계정, 기능 목록, 화면 플로우, Maestro 테스트 결과, 검증 상태표, 제한사항, 다음 액션 | [📄 README.md](../README.md) |
| 2 | **INDEX.md** | 출력물 인덱스 (본 파일) | [📄 INDEX.md](INDEX.md) |
| 3 | **CYCLE_REPORT.md** | 사이클 리포트 — Cycle 01 전체 요약, Gate 통과 현황, 화면 플로우, 핵심 성과 | [📄 CYCLE_REPORT.md](cycle-01/CYCLE_REPORT.md) |
| 4 | **QA_RESULTS.md** | QA 체크리스트 — 13개 검증 항목별 상세 결과, 스크린샷, 비고 | [📄 QA_RESULTS.md](cycle-01/QA_RESULTS.md) |
| 5 | **MAESTRO_EXECUTION_REPORT.md** | Maestro 실행 보고서 — 실행 환경, Flow 상세, Step별 결과, 이슈 분석 | [📄 MAESTRO_EXECUTION_REPORT.md](cycle-01/MAESTRO_EXECUTION_REPORT.md) |
| 6 | **VERIFICATION_STATUS.md** | 검증 상태표 — Cycle 01 전체 검증 항목 상태 매트릭스 | [📄 VERIFICATION_STATUS.md](cycle-01/VERIFICATION_STATUS.md) |
| 7 | **CHANGE_SUMMARY.md** | 변경 요약 — 생성/수정된 파일 목록 및 설명 | [📄 CHANGE_SUMMARY.md](cycle-01/CHANGE_SUMMARY.md) |
| 8 | **REVIEW_REQUEST.md** | 리뷰 요청 — Cycle 01 결과에 대한 공식 리뷰 요청서 | [📄 REVIEW_REQUEST.md](cycle-01/REVIEW_REQUEST.md) |
| 9 | **스크린샷 갤러리** | 앱 6개 화면 + Maestro 실행 결과 스크린샷 (총 7장) | [📸 스크린샷](../screenshots/) |

## 참고 자료

| 자료 | 설명 | 위치 |
|------|------|------|
| 프로젝트 설정 | XcodeGen 프로젝트 설정 파일 | [project.yml](../project.yml) |
| 메인 Maestro Flow | 전체 E2E 예약 플로우 | [maestro-flows/login-reservation-flow.yaml](../maestro-flows/login-reservation-flow.yaml) |
| 앱 소스 코드 | SwiftUI 소스 파일 | [Sources/ReservationApp/](../Sources/ReservationApp/) |
| 데이터 모델 | Service, Reservation 모델 | [Models/ReservationModel.swift](../Sources/ReservationApp/Models/ReservationModel.swift) |
| 스크린샷 디렉토리 | 앱 화면 6장 + Maestro 실행 결과 1장 | [screenshots/](../screenshots/) |

## 문서 상태

| 항목 | 값 |
|------|-----|
| 생성일 | 2026-05-06 |
| 작성자 | Hermes Agent |
| 프로젝트 | mobile-reservation-maestro-portfolio |
| Cycle | 01 |
| 상태 | ✅ 완료 |
